...
export async function obtenerParametrosDefault(): Promise<ParametrosSimulacionType> {
  ...
  if (!response.ok) {
    throw new Error("Backend no disponible. Por favor, asegúrate de que el servidor Python esté ejecutándose.")
  }

  const data = await response.json()
  console.log("[v0] Parámetros recibidos del backend:", data)
  return data
}
